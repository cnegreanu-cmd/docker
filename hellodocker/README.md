...
|---|---|
| `services` |Define el contenedor o conjunto de contenedores que formarán nuestra aplicación. |
| `image` |  Especifica la imagen de Docker a partir de la cual se creará y levantará el contenedor|
| `restart` |Define la política de reinicio del contenedor si este se detiene inesperadamente |
| `ports` |Configura el mapeo de puertos entre la máquina host y el contenedor con el formato |
| `environment` |Permite definir variables de entorno que se inyectarán dentro del contenedor para configurar la aplicación |
| `volumes` (dentro del servicio) |Monta una ruta del host o un volumen nombrado en una ruta específica dentro de ese contenedor concreto para persistir datos |
| `volumes` (al final del fichero) |Declara formalmente los volúmenes globales compartidos o independientes que usarán los servicios del fichero |

1. **¿Por qué basta con escribir `db`?**
   Porque Docker crea una red interna donde el nombre del servicio funciona automáticamente como su dirección de internet
2. **¿Sigue instalado WordPress tras `down` y `up`?**
Sí porque l comando down borra los contenedores, pero los volúmenes guardan los datos para que el nuevo contenedor los lea al hacer up
### Parte 3 · Fase 1

1. **`depends_on`:**
2. **¿Por qué `db` y no `localhost`?**
Porque dentro de un contenedor, localhost significa "este mismo contenedor". Para hablar con otro contenedor, debes llamarlo por su nombre
### Parte 3 · Fase 2

1. **`PMA_HOST`, `PMA_USER`, `PMA_PASSWORD`:**
2. **Contenedores recreados:**
   Al actualizar el fichero y hacer up, Docker solo recrea los contenedores modificados o afectados por cambios.
3. **Riesgo del login automático:**
4. Cualquiera que acceda a la web entrará directo a la base de datos como administrador, sin pedirle contraseña.
